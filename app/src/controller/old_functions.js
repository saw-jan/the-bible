...
function loadBible() {
  ...
  knex("verses")
    .select("verse")
    .where({ book_id: book, chapter: chapter })
    .then((rows) => {
      for (row of rows) {
  ...
  knex
    .select("book_name", "chapter", "verse", "scripture")
    .from("verses")
    .innerJoin("books", "verses.book_id", "books.book_id")
    .where({ "verses.book_id": book, chapter: chapter })
    .then((rows) => {
      let tVerse = 0;
      for (row of rows) {
