...
function getScripture() {
  conn.runAsync(
    "SELECT book_name,chapter,verse,scripture FROM verses INNER JOIN books ON verses.book_id=books.book_id WHERE verses.book_id = ? AND chapter=?",
    [book, chapter],
    function (rows) {
      let tVerse = 0;
      for (row of rows) {
        scriptures.innerHTML +=
          "<p id='v" + row.verse + "'><sup>" + isNep(row.verse) + "</sup> <span class='intend-txt'>" + row.scripture + "</span></p>";
        tVerse = row.verse;
      }
...
function plusFont() {
  ...
  if (scriptureFont > 17 && scriptureFont <= 29) {
    $("#m-plus img").css("opacity", 0.8);
    $("#m-minus img").css("opacity", 0.8);
    $(".scripture p").css("fontSize", scriptureFont);
    $(".scripture sup").css("fontSize", supFont);
  }
  if (scriptureFont >= 29) {
